...
public interface ScoreDAO {
    ...
    @Query("SELECT * FROM scores WHERE niveau = :niveau AND mode = :mode AND temps = :temps")
    List<Score> getScores(int niveau, String mode, long temps);

    @Query("SELECT * FROM scores WHERE niveau = :niveau AND mode = :mode")
    List<Score> getNotFixedTimeModeScores(int niveau, String mode);

    @Query("SELECT * FROM scores WHERE niveau = :niveau AND mode = :mode AND temps = :temps ORDER BY score DESC, date ASC LIMIT 1")
    Score getWorstScore(int niveau, String mode, long temps);

    @Query("SELECT * FROM scores WHERE niveau = :niveau AND mode = :mode ORDER BY score DESC, temps ASC, date ASC LIMIT 1")
